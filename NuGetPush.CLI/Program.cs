...
{	
	...
	if (File.Exists(packageFullPath)) File.Delete(packageFullPath);

	//var packResult = RunProcess(path, "dotnet", "pack");
	//if (!packResult.Success) throw new Exception($"dotnet pack failed: {packResult.Message}");
...
static (bool Success, string Message) RunProcess(string workingDirectory, string exe, string arguments)
{
